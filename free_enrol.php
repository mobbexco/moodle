...
$data->mobbexEmail = required_param('email', PARAM_EMAIL);
...
if (! $user = $DB->get_record("user", array("id" => $data->userid))) {
    message_mobbexpayment_error_to_admin("Not a valid user id", $data);
    redirect($CFG->wwwroot);
}

if (! $course = $DB->get_record("course", array("id" => $data->courseid))) {
    message_mobbexpayment_error_to_admin("Not a valid course id", $data);
    redirect($CFG->wwwroot);
}

if (! $context = context_course::instance($course->id, IGNORE_MISSING)) {
    message_mobbexpayment_error_to_admin("Not a valid context id", $data);
    redirect($CFG->wwwroot);
}
...
if (! $plugininstance = $DB->get_record("enrol", array("id" => $data->instanceid, "status" => 0))) {
    message_mobbexpayment_error_to_admin("Not a valid instance id", $data);
    redirect($CFG->wwwroot);
}
...
if ($data->courseid != $plugininstance->courseid) {
    message_mobbexpayment_error_to_admin("Course Id does not match to the course settings, received: ".$data->courseid, $data);
    redirect($CFG->wwwroot);
}
...
try {
    ...
    $checkcustomer = $DB->get_records('enrol_mobbexpayment',
    array('receiver_email' => $data->mobbexEmail));
    ...
    if (!$checkcustomer) {
        ...
        "description" => get_string('charge_description1', 'enrol_mobbexpayment'));
        if ($iscoupon) {
    ...
    if (is_enrolled($context, null, '', true)) { // TODO: use real mobbex check.
